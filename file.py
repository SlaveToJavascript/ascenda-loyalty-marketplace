...
all_partner_codes = requests.get("https://api.cs301-g7.com/loyaltyprogram/partnercodes", headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
PARTNERCODEMAPPING = all_partner_codes.json()["PARTNERCODEMAPPING"]

LOYALTYPARTNERURLMAPPING = {
    "GP": "https://api.cs301-g7.com/receive_accrual",
    "IP": "https://api.cs301-g7.com/receive_accrual"
}
...
@app.route("/addTransferDetails", methods=['PUT'])
def addTransfer():
    status = 400
    ...
    r = requests.post(validationURL + "accrual", json = result, headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
...
#retrieving content of handback 
@app.route("/file/retrieve_handback", methods=['POST'])
...
def send_accrual():
    ...
    if (results["status"] != 200):
        ...
        for loyaltyPartner in new_dict_result:
            ...
            try:
                r = requests.put(loyaltyPartnerURL, files=files, headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
                print(r)
