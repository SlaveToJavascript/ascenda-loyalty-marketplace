...
CORS(app)
validationURL = " https://z8lnzkjnyf.execute-api.us-east-1.amazonaws.com/production/validation/"
EXTENSION = ".txt"

all_partner_codes = requests.get(" https://z8lnzkjnyf.execute-api.us-east-1.amazonaws.com/production/loyaltyprogram/partnercodes", headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
PARTNERCODEMAPPING = all_partner_codes.json()["PARTNERCODEMAPPING"]

LOYALTYPARTNERURLMAPPING = {
    "GP": "Gopointsthirdparty-env.eba-rya8ymsd.us-east-1.elasticbeanstalk.com",
    "IP": "Indopointsthirdparty-env.eba-kgewyjh5.us-east-1.elasticbeanstalk.com"
}
...
def connectDynamoDBClient():
    ...
    try:
        response_table = client.describe_table(TableName='ITSA-US-EAST-1-FILE-DB')
...
def addTransfer():
    ...
    if (not (request.is_json)):
        ...
        replymessage = json.dumps({"status": status, "message": "Membership information should be in JSON", "data": result}, default=str, headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
        return replymessage

    result = request.get_json()
    result = json.loads(json.dumps(result, default=str))
    r = requests.post(validationURL + "accrual", json = result)
...
def receive_handback():
    ...
    r = requests.post(validationURL + "handback", files = {"handback_file": handback_file}, headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
    ...
    if (results["status"] != 200):
        ...
        for loyaltyPartner in new_dict_result:
            ...
            with open("./file/" + filename, "rb") as f:
                try:
                    s3, bucket_name = connectS3Client()
                    s3.upload_fileobj(f, "itsa-us-east-1-file-s3", filename)
                except botocore.exceptions.ClientError as error:
                    return {"status": 400, "message": "Upload to S3 bucket failed"}
            ...
            try:
                r = requests.post(loyaltyPartnerURL, files=files, headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
                print(r)
