...
def connectS3Resource():
    ...
    try:
        s3.head_bucket(Bucket=bucket_name)
        s3 = boto3.resource('s3', region_name="us-east-1",  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    except ClientError:
        bucket_name = "itsa-ap-east-1-file-s3"
...
def validateAccrual():
    ...
    programExistResponse = requests.get(" https://z8lnzkjnyf.execute-api.us-east-1.amazonaws.com/production/loyaltyprogram/" + str(dict_result["Loyalty Program"]), headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
