...
def connectDynamoDBClient():
    ...
    try:
        response_table = client.describe_table(TableName='ITSA-AP-EAST-1-VALIDATION-DB')
...
def connectDynamoDBFileClient():
        ...
        try:
            response_table_file = dynamodb.describe_table(TableName='ITSA-AP-EAST-1-FILE-DB')
...
def connectS3Client():
    ...
    try:
        ...
        s3 = boto3.client('s3', region_name="ap-east-1",  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    except ClientError:
        bucket_name = "itsa-us-east-1-file-s3"
        s3_client = boto3.client('s3', region_name="us-east-1",  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    return s3
...
def connectS3Resource():
    ...
    try:
        s3.head_bucket(Bucket=bucket_name)
        s3 = boto3.resource('s3', region_name="ap-east-1",  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    except ClientError:
        bucket_name = "itsa-us-east-1-file-s3"
        s3_client = boto3.resource('s3', region_name="us-east-1",  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    return s3
...
def validateAccrual():
    ...
    programExistResponse = requests.get("https://aljzpgguk3.execute-api.ap-east-1.amazonaws.com/production/loyaltyprogram/" + str(dict_result["Loyalty Program"]), headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
...
def get_transactions_by_member(memberFirstName,memberLastName):
    file_table, dynamodb = connectDynamoDBFileClient()
...
def delete_PII_file(reference_number):
    file_table, client = connectDynamoDBFileClient()
...
def delete_accrual_by_user(memberFirstName, memberLastName, bankID):
    ...
    if(s3.Bucket('itsa-ap-east-1-file-s3') in s3.buckets.all()):
