...
engine = create_engine("mysql://admin:cs301team7!@gopoints.cvxifbjfrdco.us-east-1.rds.amazonaws.com:3306/indopoints", echo=False, future=True)
...
def processFile(filePrefix):
    ...
    for file in os.listdir(uploadFolder):
        ...
        try :
            res = requests.put("https://api.cs301-g7.com/file/handback", files = {"handback_file": handbackFile})
        except:
            res = requests.put("http://localhost:5002/file/handback", files = {"handback_file": handbackFile})
        print(res)
    print("Complete.")
...
if not os.path.exists('./IPuploads'):
    os.makedirs('./IPuploads')
if not os.path.exists('./IPhandbacks'):
    os.makedirs('./IPhandbacks')
if not os.path.exists('./IPprocessed'):
    os.makedirs('./IPprocessed')
