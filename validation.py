...
def connectAPS3Client(region):
    s3_client = boto3.client('s3', aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    if (region == 'ap-east-1'):
        s3_client = boto3.client('s3', region_name=region,  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    return s3_client

def connectAPS3Resource(region):
    ...
    if(region == 'ap-east-1'):
        s3 = boto3.resource('s3', region_name=region,  config=boto3.session.Config(s3={'addressing_style': 'virtual'}, signature_version='s3v4'),aws_access_key_id='AKIAWGZOGTL2WLBFP5IB',aws_secret_access_key='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')
    return s3
...
def validateAccrual():
    ...
    programExistResponse = requests.get("https://api.cs301-g7.com/loyaltyprogram/" + str(dict_result["Loyalty Program"]), headers={"X-API-KEY": "bv34XeV2TN1ta2ZAuTXlqU5eA0piseRBbHJCXih0"})
...
def delete_contents(bucket, file_name, reference_numbers, region):
    #read the file
    s3_client = connectAPS3Client(region)
...
def delete_accrual_by_user(memberFirstName, memberLastName, bankID):
    ...
    if(s3.Bucket('itsa-us-east-1-file-s3') in s3.buckets.all() and s3.Bucket('itsa-ap-east-2-file-s3') in s3.buckets.all()):
    ...
    if(s3.Bucket('itsa-us-east-1-file-s3') in s3.buckets.all()):
        ...
        return {"status": 400, "message":"itsa-us-east-1 bucket does not exist"}
    ...
    if(s3.Bucket('itsa-ap-east-1-file-s3') in s3.buckets.all()):
        ...
        return {"status": 400, "message":"itsa-ap-east-1 bucket does not exist"}
